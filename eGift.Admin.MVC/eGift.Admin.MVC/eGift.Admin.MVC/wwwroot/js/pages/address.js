...
$(document).ready(function () {
    ...
    var _cityId = $('#CityId').attr('data-cityId');

    // Call Bind State By Country Id
    BindStateByCountryId(_countryId, _stateId);

    // Call Bind Cities By State Id
    BindCitiesByStateId(_stateId, _cityId);
    ...
    // City List On State Change
    $('#StateId').change(function () {
        var stateId = $(this).val();

        // Bind Cities By State Id
        BindCitiesByStateId(stateId, 0);
    });

    // Delete Address
    $('#address-delete-button').on('click', function (event) {
        swal({
            title: 'Are you sure?',
...
function BindStateByCountryId(countryId, stateId) {
    $.ajax({
        url: '/Address/GetStatesByCountry',
        type: 'GET',
...
// Bind Cities By State Id
function BindCitiesByStateId(stateId, cityId) {
    $.ajax({
        url: '/Address/GetCitiesByState',
        type: 'GET',
